...
- 🎯 Entry points to execute your program using `python -m <sanit2>` or `$ sanit2` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/engrogerio/sanit2/branch/main/graph/badge.svg?token=sanit2_token_here)](https://codecov.io/gh/engrogerio/sanit2)
[![CI](https://github.com/engrogerio/sanit2/actions/workflows/main.yml/badge.svg)](https://github.com/engrogerio/sanit2/actions/workflows/main.yml)

Awesome sanit2 created by engrogerio

## Install

from source
```bash
git clone https://github.com/engrogerio/sanit2 sanit2
cd sanit2
make install
```
...
```bash
$ sanit2 run --port 8080
```

or

```bash
python -m sanit2 run --port 8080
```
...
```bash
❯ sanit2 --help
Usage: sanit2 [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ sanit2 create-user --help
Usage: sanit2 create-user [OPTIONS] USERNAME PASSWORD
...
```bash
sanit2 create-user admin admin --superuser
```
...
Run with `sanit2 run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `sanit2 create-user`.
...
-----------------------------------------------------
sanit2/__init__.py              4      0   100%
sanit2/app.py                  16      1    94%
sanit2/cli.py                  21      0   100%
sanit2/config.py                5      0   100%
sanit2/db.py                   10      0   100%
sanit2/models/__init__.py       0      0   100%
sanit2/models/content.py       47      1    98%
sanit2/routes/__init__.py      11      0   100%
sanit2/routes/content.py       52     25    52%
sanit2/routes/security.py      15      1    93%
sanit2/routes/user.py          52     26    50%
sanit2/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in sanit2/default.toml.
